...
var u = (r3, t3, e2) => t3 in r3 ? f(r3, t3, { enumerable: true, configurable: true, writable: true, value: e2 }) : r3[t3] = e2;
var c = (r3, t3, e2) => u(r3, typeof t3 != "symbol" ? t3 + "" : t3, e2);
function a(r3, t3) {
  return (e2) => e2 instanceof r3 && e2.type === t3;
}
function p(r3, t3) {
  t3 || (t3 = []);
  const e2 = Symbol(r3);
  class n2 extends Error {
    constructor(...i) {
      const o3 = typeof t3 == "function" ? t3(...i) : typeof t3 == "string" ? [t3] : t3 || [];
      super(...o3);
      c(this, "type", e2);
      this.name = r3;
    }
  }
  return Object.defineProperty(n2, "name", { value: r3 }), [n2, a(n2, e2)];
}
function l(r3, t3, e2) {
  const n2 = Symbol(r3);
  class s2 extends p(r3, e2)[0] {
    constructor(...o3) {
      super(...o3);
      c(this, "data");
      c(this, "type", n2);
  ...
  return Object.defineProperty(s2, "name", { value: r3 }), [s2, a(s2, n2)];
}

// node_modules/@telegram-apps/init-data-node/dist/entries/shared-aDQj8NQw.js
...
function signData(data, key, createHmac2, options = {}) {
  const keyHmac = options.tokenHashed ? typeof key === "string" ? hexToArrayBuffer(key) : key : hashToken(key, createHmac2);
  if (keyHmac instanceof Promise) {
    return keyHmac.then((v) => createHmac2(data, v)).then(arrayBufferToHex);
  }
  const hmac = createHmac2(data, keyHmac);
...
] = p("SignatureInvalidError");
var [
  SignatureMissingError,
  isSignatureMissingError
] = p("SignatureMissingError", (thirdParty) => [
...
function validate(value, token2, signData22, options = {}) {
  let authDate;
  let authDateString;
  let hash;
  const pairs = [];
  ...
  pairs.sort();
  const sign2 = signData22(pairs.join("\n"), token2, options);
  return typeof sign2 === "string" ? processSign(sign2, hash) : sign2.then((v) => processSign(v, hash));
}

// node_modules/@telegram-apps/init-data-node/dist/entries/web.js
...
function signData2(data, key, options) {
  return signData(data, key, createHmac, options);
}
function validate2(value, token2, options) {
  return Promise.resolve().then(() => validate(value, token2, signData2, options));
}

// node_modules/itty-router/index.mjs
var t = ({ base: e2 = "", routes: t3 = [], ...r3 } = {}) => ({ __proto__: new Proxy({}, { get: (r4, o3, a2, s2) => (r5, ...c3) => t3.push([o3.toUpperCase?.(), RegExp(`^${(s2 = (e2 + r5).replace(/\/+(\/|$)/g, "$1")).replace(/(\/?\.?):(\w+)\+/g, "($1(?<$2>*))").replace(/(\/?\.?):(\w+)/g, "($1(?<$2>[^$1/]+?))").replace(/\./g, "\\.").replace(/(\/?)\*/g, "($1.*)?")}/*$`), c3, s2]) && a2 }), routes: t3, ...r3, async fetch(e3, ...o3) {
  let a2, s2, c3 = new URL(e3.url), n2 = e3.query = { __proto__: null };
  for (let [e4, t4] of c3.searchParams) n2[e4] = n2[e4] ? [].concat(n2[e4], t4) : t4;
  e: try {
    for (let t4 of r3.before || []) if (null != (a2 = await t4(e3.proxy ?? e3, ...o3))) break e;
    t: for (let [r4, n3, l3, i] of t3) if ((r4 == e3.method || "ALL" == r4) && (s2 = c3.pathname.match(n3))) {
      e3.params = s2.groups || {}, e3.route = i;
      for (let t4 of l3) if (null != (a2 = await t4(e3.proxy ?? e3, ...o3))) break t;
    }
  } catch (t4) {
    if (!r3.catch) throw t4;
    a2 = await r3.catch(t4, e3.proxy ?? e3, ...o3);
  }
  try {
    for (let t4 of r3.finally || []) a2 = await t4(a2, e3.proxy ?? e3, ...o3) ?? a2;
  } catch (t4) {
    if (!r3.catch) throw t4;
    a2 = await r3.catch(t4, e3.proxy ?? e3, ...o3);
  }
...
var r = (e2 = "text/plain; charset=utf-8", t3) => (r3, o3 = {}) => {
  if (void 0 === r3 || r3 instanceof Response) return r3;
  const a2 = new Response(t3?.(r3) ?? r3, o3.url ? void 0 : o3);
  return a2.headers.set("content-type", e2), a2;
};
var o = r("application/json; charset=utf-8", JSON.stringify);
var p2 = r("text/plain; charset=utf-8", String);
var f2 = r("text/html");
var u2 = r("image/jpeg");
var h = r("image/png");
var g = r("image/webp");
...
function n(n2) {
  ...
  o3.unshift(-1);
  const s2 = t2(o3, 0, o3.length);
  return (e2) => r2(s2, e2);
}
function t2(e2, n2, r3) {
  if (r3 - n2 == 1) return { offset: e2[n2], index: n2 + 1 };
  const o3 = Math.ceil((n2 + r3) / 2), s2 = t2(e2, n2, o3), l3 = t2(e2, o3, r3);
...
function o2(e2, t3 = "", r3 = {}) {
  const o3 = "string" != typeof t3 ? t3 : r3, l3 = "string" == typeof t3 ? t3 : "", c3 = e2.map(s), f3 = !!o3.lineNumbers;
  return function(e3, t4 = 0) {
    ...
    e: for (; o4 < e3.length; ) {
      let n2 = false;
      for (const t5 of c3) {
        t5.regex.lastIndex = o4;
        const c4 = t5.regex.exec(e3);
        if (c4 && c4[0].length > 0) {
          if (!t5.discard) {
            const e4 = r4(o4), n3 = "string" == typeof t5.replace ? c4[0].replace(new RegExp(t5.regex.source, t5.regex.flags), t5.replace) : c4[0];
            s2.push({ state: l3, name: t5.name, text: n3, offset: o4, len: c4[0].length, line: e4.line, column: e4.column });
          }
          if (o4 = t5.regex.lastIndex, n2 = true, t5.push) {
            const n3 = t5.push(e3, o4);
            s2.push(...n3.tokens), o4 = n3.offset;
          }
          if (t5.pop) break e;
          break;
        }
...
function map(p3, mapper) {
  return (data, i) => mapInner(p3(data, i), (v, j) => mapper(v, data, i, j));
}
...
function flatten1(p3) {
  return map(p3, (vs) => vs.flatMap((v) => v));
}
...
function reduceLeft(acc, p3, reducer) {
  return chainReduce(acc, (acc2) => map(p3, (v, data, i, j) => reducer(acc2, v, data, i, j)));
}
function leftAssoc2(pLeft, pOper, pRight) {
  return chain(pLeft, (v0) => reduceLeft(v0, ab(pOper, pRight, (f3, y) => [f3, y]), (acc, [f3, y]) => f3(acc, y)));
}
...
function limitedDepthRecursive(n2, f3, g2 = () => void 0) {
  if (n2 === void 0) {
  ...
  if (n2 >= 0) {
    return function(...args) {
      return f3(limitedDepthRecursive(n2 - 1, f3, g2), ...args);
    };
  }
  return g2;
}
...
function getHex(c3) {
  if (c3 >= 48 && c3 <= 57 || c3 >= 97 && c3 <= 102 || c3 >= 65 && c3 <= 70) {
  ...
  if (encoding === "Q") {
    ...
    for (let i = 0, len = buf.length; i < len; i++) {
      ...
      if (i <= len - 2 && c3 === 61) {
        ...
        let c22 = getHex(buf[i + 2]);
        if (c1 && c22) {
          let c4 = parseInt(c1 + c22, 16);
          encodedBytes.push(c4);
          i += 2;
          continue;
        }
      }
...
function decodeURIComponentWithCharset(encodedStr, charset) {
  ...
  for (let i = 0; i < encodedStr.length; i++) {
    let c3 = encodedStr.charAt(i);
    if (c3 === "%" && /^[a-f0-9]{2}/i.test(encodedStr.substr(i + 1, 2))) {
      let byte = encodedStr.substr(i + 1, 2);
      i += 2;
      encodedBytes.push(parseInt(byte, 16));
    } else if (c3.charCodeAt(0) > 126) {
      c3 = textEncoder.encode(c3);
      for (let j = 0; j < c3.length; j++) {
        encodedBytes.push(c3[j]);
      }
    } else {
      encodedBytes.push(c3.charCodeAt(0));
    }
  }
  ...
  feed(line) {
    switch (this.state) {
      case "header":
        ...
        this.headerLines.push(getDecoder2().decode(line));
        break;
      case "body": {
...
function escapeHtml(str) {
  return str.trim().replace(/[<>"'?&]/g, (c3) => {
    let hex = c3.charCodeAt(0).toString(16);
    if (hex.length < 2) {
      hex = "0" + hex;
    }
...
function base64ArrayBuffer(arrayBuffer) {
  ...
  for (var i = 0; i < mainLength; i = i + 3) {
    ...
    c3 = (chunk & 4032) >> 6;
    d = chunk & 63;
    base64 += encodings[a2] + encodings[b] + encodings[c3] + encodings[d];
  }
  if (byteRemainder == 1) {
    ...
    b = (chunk & 1008) >> 4;
    c3 = (chunk & 15) << 2;
    base64 += encodings[a2] + encodings[b] + encodings[c3] + "=";
  }
  return base64;
}
    ...
    for (const key of ["delivered-to", "return-path"]) {
      ...
      if (addressHeader && addressHeader.value) {
        ...
        if (addresses && addresses.length && addresses[0].address) {
          const camelKey = key.replace(/\-(.)/g, (o3, c3) => c3.toUpperCase());
...
[Truncated] The original size of the email was ${message.rawSize} bytes, which exceeds the maximum size of ${maxSize} bytes.`;
    ...
    const msg = `Error parsing email: ${e2.message}`;
    cache.text = msg;
    cache.html = msg;
  }
  return cache;
...
${mail.text}`;
  try {
    ...
    req.text = `Failed to summarize the email: ${e2.message}`;
  }
  return req;
}
...
function createTmaAuthMiddleware(env) {
  ...
  return async (req) => {
    ...
    try {
      await validate2(authData, TELEGRAM_TOKEN, {
        expiresIn: 3600
      });
      ...
      throw new HTTPError(401, e2.message);
    }
  };
}
  ...
  globalThis.Buffer = class Buffer2 extends ArrayBuffer {
    ...
    static from(data, encoding) {
      ...
      if (data instanceof Uint8Array) {
        return new Buffer2(data.buffer);
      }
