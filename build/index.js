...
function decodeURIComponentWithCharset(encodedStr, charset) {
  ...
  for (let i2 = 0; i2 < encodedStr.length; i2++) {
    let c3 = encodedStr.charAt(i2);
    if (c3 === "%" && /^[a-f0-9]{2}/i.test(encodedStr.substr(i2 + 1, 2))) {
  ...
  async processNodeTree() {
    ...
    let walk = async (node, alternative, related) => {
      ...
      if (!node.contentType.multipart) {
        ...
        } else if ((/^text\//i.test(node.contentType.parsed.value) || node.contentType.parsed.value === "message/delivery-status") && node.contentDisposition.parsed.value !== "attachment") {
  ...
  async parse(buf) {
    ...
    for (let key of ["from", "sender", "reply-to"]) {
      ...
      if (addressHeader && addressHeader.value) {
        ...
        if (addresses && addresses.length) {
          message[key.replace(/\-(.)/g, (o3, c3) => c3.toUpperCase())] = addresses[0];
        }
      }
    }
    for (let key of ["delivered-to", "return-path"]) {
      ...
      if (addressHeader && addressHeader.value) {
        ...
        if (addresses && addresses.length && addresses[0].address) {
          message[key.replace(/\-(.)/g, (o3, c3) => c3.toUpperCase())] = addresses[0].address;
        }
      }
    }
    ...
    for (let key of ["subject", "message-id", "in-reply-to", "references"]) {
      ...
      if (header && header.value) {
        message[key.replace(/\-(.)/g, (o3, c3) => c3.toUpperCase())] = decodeWords(header.value);
      }
    }
...
async function parseEmail(message) {
  ...
  try {
    ...
    const msg = `Error parsing email: ${e3.message}`;
    cache.text = msg;
    cache.html = msg;
  }
  return cache;
